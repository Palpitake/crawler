...
from collection_evidence import (
...
MCP_BROWSER_TOOLS_BUILD = "2026.07.22-mcp-v4.3-runtime-facts"
...
def _validate_auth_state_file(path: Path) -> Dict[str, Any]:
    try:
        ...
        if not isinstance(cookies, list) or not isinstance(origins, list):
            return {"ok": False, "error": "登录态文件格式无效"}
        if not cookies and not origins:
            return {"ok": False, "error": "登录态为空，未检测到 cookie 或 localStorage"}
        now = time.time()
...
def _login_probe_is_complete(probe: Dict[str, Any]) -> bool:
    if not probe.get("ok"):
        return False
    blocked = (
        int(probe.get("password_inputs", 0) or 0) > 0
        or bool(probe.get("hard_gate"))
        or int(probe.get("auth_overlays", 0) or 0) > 0
    )
    href = str(probe.get("href", ""))
    on_auth_path = bool(re.search(r"(?:^|[/._-])(login|signin|sign-in|auth)(?:[/._?&#-]|$)", href, re.I))
    authenticated_hint = int(probe.get("authenticated_hints", 0) or 0) > 0
    has_content = int(probe.get("body_text_chars", 0) or 0) >= 300
    return bool(not blocked and has_content and (not on_auth_path or authenticated_hint))
...
def browser_auth_probe(target_url: Optional[str] = None) -> Dict[str, Any]:
...
class ManualLoginArgs(AuthStateArgs):
    ...
    timeout_seconds: int = Field(300, description="Maximum time for the user to finish login")
    poll_interval_seconds: float = Field(2.0, description="Login completion polling interval")
    require_confirmation: bool = Field(True, description="Require the user to confirm login in the console")
...
def browser_manual_login(
    ...
    timeout_seconds = max(15, min(int(timeout_seconds), 1800))
    poll_interval_seconds = max(0.5, min(float(poll_interval_seconds), 10.0))
    ...
    try:
        ...
        def _persist_confirmed_login(
            ...
            post_login_probe = _login_probe() if navigate_after.get("ok") else {
                "ok": False,
        ...
        # Optional unattended mode retained for non-interactive deployments.
        ...
        while time.monotonic() < deadline:
            try:
                last_probe = _login_probe()
                if not last_probe.get("ok"):
                    consecutive_probe_errors += 1
                    if consecutive_probe_errors >= 3:
                        return {
                            "ok": False,
                            "error_code": "login_browser_closed",
                            "error": last_probe.get("error", "登录窗口已关闭或浏览器连接中断"),
                        }
                    time.sleep(poll_interval_seconds)
                    continue
                consecutive_probe_errors = 0
                if _login_probe_is_complete(last_probe):
                    stable_clear += 1
                    if stable_clear >= 2:
                        return _persist_confirmed_login(last_probe, confirmed_by_user=False)
                else:
                    stable_clear = 0
            except Exception as exc:
                consecutive_probe_errors += 1
                log_event(logger, "auth.probe", level="DEBUG", status="failed", agent="browser", error_type="auth_probe_failed", reason=str(exc))
                if consecutive_probe_errors >= 3:
                    return {
                        "ok": False,
                        "error_code": "login_browser_closed",
                        "error": f"登录窗口已关闭或浏览器连接中断: {exc}",
                    }
            time.sleep(poll_interval_seconds)
        ...
        return {
            ...
            "error": f"等待人工登录超时（{timeout_seconds} 秒）",
            "challenge_detected": int(last_probe.get("challenge_hints", 0) or 0) > 0,
        }
    except Exception as exc:
...
def browser_clear_auth_state(session_name: str = "default", state_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if existed:
            path.unlink()
        return {"ok": True, "session_name": _safe_session_name(session_name), "deleted": existed}
    except Exception as exc:
