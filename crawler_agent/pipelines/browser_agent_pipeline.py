...
from crawler_agent.auth.contracts import build_verification_contract
from crawler_agent.auth.models import AuthState
from crawler_agent.auth.sessions import (
    AuthSessionStore,
    browser_environment_fingerprint,
    scoped_session_name,
)
from crawler_agent.core.common import write_json_atomic
from crawler_agent.core.logger import get_logger, log_event
from crawler_agent.core.runtime_facts import (
    endpoint_provenance,
    normalize_auth_facts,
    page_partition,
    sanitize_url,
)
from crawler_agent.core.transcript_utils import sanitize_agent_transcript
from crawler_agent.rag.feedback import browser_memory_receipts, emit_memory_feedback
from crawler_agent.runtimes.pi_browser_runtime import (
    PiRuntimeUnavailable,
    run_pi_browser_agent,
)
from crawler_agent.version import BUILD_DATE, VERSION


logger = get_logger("agent.browser")
BROWSER_AGENT_BUILD = f"{BUILD_DATE}-browser-native-loop-v{VERSION}"
...
17. 若 Memory Card 实际影响了你的端点选择、交互或验证方向，在 submit_parser.memory_usage 中记录对应 memory_id、status=used 和原因；若当前证据否定了它，记录 status=rejected。仅仅看到卡片不得申报 used。
18. 对同站点、同路由且高分的 endpoint/strategy Memory Card，先做一次有界验证：优先按 endpoint family 过滤当前网络日志并读取最强响应体；验证失败立即标 rejected 并回到普通探索。跨站策略只能提示通用分页思路，禁止把其 URL、selector 或认证事实用于当前站点。
...
def _try_resume_browser_context(
    ...
    try:
        from crawler_agent.tools.mcp_browser_tools import browser_evaluate
...
def _load_auth_infrastructure(
    ...
    from crawler_agent.tools.mcp_browser_tools import (
        browser_load_auth_state,
        configure_browser_mcp,
    )

    environment_fingerprint = browser_environment_fingerprint()
    resolved = _safe_session(session_name) if session_name else scoped_session_name(target_url)
    state_path = _auth_directory() / f"{resolved}.json"
    session_store = AuthSessionStore(_auth_directory())
    load_decision = session_store.can_load(
        resolved,
    ...
    phase: Dict[str, Any] = {
        ...
        "environment_fingerprint": environment_fingerprint,
        "session_lifecycle": load_decision.metadata,
        "error": None,
    }
    if skip_saved or not load_decision.allowed:
        configure_browser_mcp(None, headless=True, restart=True)
        blocked_status = str(load_decision.metadata.get("status") or "stale")
        phase.update({
            "state_quarantined": not load_decision.allowed,
            "authentication_state": "anonymous",
            "saved_session_state": blocked_status if not load_decision.allowed else None,
            "error": None if skip_saved else load_decision.reason,
            "recovery": {
                "reason": "saved_state_skipped" if skip_saved else load_decision.reason,
                "metadata": load_decision.metadata,
            },
        })
        return None, phase
    if not should_load:
    ...
    try:
        ...
        if isinstance(loaded, dict) and loaded.get("ok"):
            try:
                lifecycle = session_store.record_loaded(
                    resolved,
                    target_url=target_url,
                    environment_fingerprint=environment_fingerprint,
                    diagnostics=loaded.get("state_diagnostics") or {},
                )
            except OSError:
                lifecycle = load_decision.metadata
            phase.update({
                "state_loaded": True,
                "session_name": resolved,
        ...
        if isinstance(loaded, dict) and loaded.get("stale_hint"):
            try:
                lifecycle = session_store.record_verification(
                    resolved,
                    state=AuthState.STALE,
                    target_url=target_url,
                    environment_fingerprint=environment_fingerprint,
                    reason_codes=["storage_state_expired"],
                )
                phase.update({
                    "authentication_state": "anonymous",
                    "saved_session_state": "stale",
                    "verification_state": "unverified",
                    "state_quarantined": lifecycle.get("status") == "quarantined",
                    "state_rejected": True,
                    "session_lifecycle": lifecycle,
                })
            except OSError:
...
def _full_task_prompt(
    ...
    if operation_mode == "resolve_authentication":
        if required_action == "revalidate_saved_session":
            return f"""Revalidate the already loaded saved authentication session.
Target URL: {target_url}
Target fields: {json.dumps(target_fields, ensure_ascii=False)}

Follow this exact protocol:
1. Call browser_open to navigate to the authorized target URL.
2. Call browser_auth_probe to collect raw target-page authentication evidence.
3. Call submit_parser with only the observed authentication facts.
Do not request manual login, explore unrelated pages, or change browser fingerprint settings.
"""
...
Memory 使用回执规则：只有实际采用或证伪的卡片才写入 memory_usage；未使用的卡片不要填写。
...
def run_browser_agent_pipeline(
    ...
    auth_contract = build_verification_contract(target_url, target_fields)
    auth_environment = browser_environment_fingerprint()
    auth_session_store = AuthSessionStore(_auth_directory())
    ...
    if page_reused:
        ...
        auth_phase = {
            ...
            "verification_contract": auth_contract.to_dict(),
            "environment_fingerprint": auth_environment,
        }
        ...
        auth_phase["verification_contract"] = auth_contract.to_dict()
    ...
    def invoke(name: str, arguments: Dict[str, Any]) -> Any:
        ...
        if name == "browser_manual_login":
            chosen_session = _safe_session(resolved_session) if resolved_session else scoped_session_name(target_url)
            ...
            if isinstance(result, dict) and result.get("ok"):
                ...
                auth_phase.update({
                    ...
                    "pre_login_probe": result.get("pre_login_probe") or {},
        ...
        if name == "browser_auth_probe":
            result = browser_auth_probe.invoke({
                "target_url": target_url,
                "target_fields": list(target_fields),
            })
            if isinstance(result, dict):
                auth_phase["post_login_probe"] = result
    ...
    try:
        browser_system_prompt = BROWSER_AGENT_SYSTEM_PROMPT
        if auth_resolution_mode and required_action == "revalidate_saved_session":
            browser_system_prompt += (
                "\nAuthoritative authentication strategy override: revalidate the loaded "
                "saved session with browser_open, then browser_auth_probe, then submit_parser. "
                "Do not call browser_manual_login in this strategy."
            )
        pi_result = run_pi_browser_agent(
            ...
            allowed_tools=(
                ["browser_open", "browser_auth_probe"]
                if auth_resolution_mode and required_action == "revalidate_saved_session"
                else ["browser_manual_login", "browser_auth_probe"]
                if auth_resolution_mode else None
            ),
    ...
    if isinstance(candidate, dict):
        emit_memory_feedback(
            logger,
            browser_memory_receipts(candidate),
            stage="parser_submission",
        )
    ...
    claimed_auth_state = str(
        auth.get("authentication_state") or auth.get("state") or ""
    ).lower()
    if (
        auth.get("auth_required")
        or claimed_auth_state in {"required", "challenge", "provisional", "verified"}
        or auth_phase.get("state_loaded")
        or auth_phase.get("attempted")
        or auth_phase.get("saved_session_state")
    ):
        host_auth_probe = browser_auth_probe.invoke({
            "target_url": target_url,
            "target_fields": list(target_fields),
        })
        if isinstance(host_auth_probe, dict):
            auth_phase["post_login_probe"] = host_auth_probe
            auth["probe"] = host_auth_probe
    ...
    auth_phase["auth_epoch"] = auth_epoch
    auth_facts = normalize_auth_facts(result, {"phases": {"login": auth_phase}})
    result["auth"] = {**auth, **auth_facts}
    if resolved_session:
        try:
            lifecycle = auth_session_store.record_verification(
                resolved_session,
                state=AuthState.from_value(auth_facts.get("state")),
                target_url=target_url,
                environment_fingerprint=auth_environment,
                reason_codes=list(auth_facts.get("reason_codes") or []),
            )
            auth_phase["session_lifecycle"] = lifecycle
            auth_phase["state_quarantined"] = lifecycle.get("status") == "quarantined"
            auth_phase["state_rejected"] = str(auth_facts.get("state") or "") in {
                "required", "challenge", "rejected", "stale",
            }
        except OSError as exc:
            auth_phase["session_lifecycle_error"] = str(exc)
    auth_phase.update({
