...
from crawler_agent.auth.models import AUTH_BLOCKING_STATES, AUTH_RESOLUTION_STATES
from crawler_agent.core.api_logger import get_all_summaries, reset_all_trackers
from crawler_agent.core.common import (
    json_dumps,
    safe_int as _safe_int,
    utc_now_iso as _now_iso,
)
from crawler_agent.core.logger import get_logger, log_event, set_log_context
...
from crawler_agent.rag import get_rag_service
...
SUPERVISOR_AGENT_BUILD = f"{BUILD_DATE}-supervisor-native-loop-v{VERSION}"
...
def _validate_parser_result(parser: Dict[str, Any], state: CrawlerState) -> Dict[str, Any]:
    ...
    if auth_facts.get("state") in AUTH_BLOCKING_STATES:
        warnings.append(f"authentication_{auth_facts.get('state')}")
    if mode == "probe":
        warnings.append("code_probe_required")

    return {
        "valid": bool(has_strategy and auth_facts.get("state") not in AUTH_BLOCKING_STATES),
        "can_enter_code": bool(state.get("target_url")),
        ...
        "need_manual_login": auth_facts.get("state") in AUTH_RESOLUTION_STATES,
...
def _commit_rag(state: CrawlerState) -> Dict[str, Any]:
    ...
    try:
        from crawler_agent.pipelines.browser_agent_pipeline import BROWSER_AGENT_BUILD
        from crawler_agent.pipelines.code_pipeline import CODE_PIPELINE_BUILD
        builds = {
...
def run_browser_capability(state: CrawlerState) -> CrawlerState:
    from crawler_agent.pipelines.browser_pipeline import run_browser_pipeline
    ...
    try:
        ...
        auth_required = auth_facts.get("state") in AUTH_BLOCKING_STATES
        if auth_required:
        ...
        if pipeline_error:
            ...
            if parser_result.get("analysis_status") == "complete" and auth_facts.get("state") not in AUTH_BLOCKING_STATES:
                error_info = {}
        ...
        next_state: CrawlerState = {
            **state,
            ...
            "need_login": "yes" if auth_facts.get("state") in AUTH_RESOLUTION_STATES else state.get("need_login", "unknown"),
            "manual_login_required": auth_facts.get("state") in AUTH_RESOLUTION_STATES,
...
def validate_parser(state: CrawlerState) -> CrawlerState:
    ...
    if auth_state in AUTH_RESOLUTION_STATES:
...
def run_code_capability(state: CrawlerState) -> CrawlerState:
    from crawler_agent.pipelines.code_pipeline import run_code_pipeline
...
def finalize(state: CrawlerState) -> CrawlerState:
    ...
    if success:
        ...
        next_action = None
    elif (error_category == "authentication" or auth_state in AUTH_RESOLUTION_STATES) and error_category not in {"access", "service", "network", "dependency"}:
    ...
    final_output["rag"] = {
        ...
        "quality": rag_commit.get("quality"),
        "failures_written": int(rag_commit.get("failures_written", 0) or 0),
        "feedback_summary": dict(rag_commit.get("feedback_summary") or {}),
...
def _state_summary(state: CrawlerState) -> Dict[str, Any]:
    ...
    memory_views = state.get("rag_memory_views") if isinstance(state.get("rag_memory_views"), dict) else {}
    top_memory_cards = []
    for card in list(memory_views.get("supervisor") or [])[:3]:
        if not isinstance(card, dict):
            continue
        top_memory_cards.append({
            "memory_id": card.get("memory_id"),
            "memory_type": card.get("memory_type"),
            "memory_domain": card.get("domain"),
            "memory_route": card.get("route_template"),
            "match_score": card.get("match_score"),
            "match_reason": card.get("match_reason"),
            "requires_validation": card.get("requires_validation"),
            "recommended_capability": card.get("recommended_capability"),
            "known_risks": list(card.get("known_risks") or [])[:5],
        })
    return {
        "thread_id": state.get("thread_id"),
        ...
        "rag_top_memory_cards": top_memory_cards,
        "rag_fast_validation_ready": bool(state.get("use_cached_strategy")),
...
def _recommended_actions(state: CrawlerState) -> List[str]:
    ...
    top_cards = list(summary.get("rag_top_memory_cards") or [])
    ...
    if blocked_failures and not summary.get("execution_attempted"):
        ...
        if auth_state in AUTH_RESOLUTION_STATES and (
            blocked_retry == "resolve_authentication" or blocked_auth_state in AUTH_RESOLUTION_STATES
        ):
    ...
    if auth_state in AUTH_RESOLUTION_STATES:
        ...
        if not summary["execution_attempted"] and not summary["parser_attempts"] and top_cards:
            recommended = str(top_cards[0].get("recommended_capability") or "")
            if recommended in {"run_browser", "run_code"}:
...
def run_supervisor(
        ...
        if capability == "set_task_spec":
            ...
            if auth_resolution:
                ...
                required_auth_action = (
                    "revalidate_saved_session"
                    if current_auth == "provisional" and active_state.get("session_name")
                    else "manual_login_and_verify"
                )
                active_state = {
                    **active_state,
                    "auth_resolution_attempts": auth_attempts + 1,
                    "browser_operation_mode": "resolve_authentication",
                    "browser_required_action": required_auth_action,
                }
            else:
                active_state = {
                    **active_state,
            ...
            if auth_resolution:
                ...
                feedback["required_action"] = active_state.get("browser_required_action")
    ...
    from crawler_agent.runtimes.pi_browser_runtime import run_pi_supervisor_agent
