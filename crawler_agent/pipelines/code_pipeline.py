...
from crawler_agent.core.common import json_dumps, write_json_atomic
...
from crawler_agent.rag.feedback import code_memory_receipts, emit_memory_feedback
...
CODE_PIPELINE_BUILD = f"{BUILD_DATE}-code-pi-coding-agent-native-v{VERSION}"
...
def _remove_stale_output(filename: str) -> bool:
    ...
    try:
        from crawler_agent.tools.code_tools import get_workspace
...
def _deterministic_code_plan(parser_result: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if session_name:
        try:
            from crawler_agent.pipelines.browser_pipeline import AUTH_DIR
...
def _read_generated_source(code_file: str) -> Dict[str, Any]:
    from crawler_agent.tools.code_tools import read_text_file
...
def _persist_code_debug_report(code_file: str, report: Dict[str, Any]) -> Dict[str, Any]:
    from crawler_agent.tools.code_tools import debug_python_file
...
- 结束前请实际读取输出文件并抽查样本，然后在最终回复最后一行输出 AI_REVIEW_JSON={"success":true/false,"confidence":0到1,"items":整数,"pagination_complete":true/false/null,"issues":[],"reason":"...","memory_ids_used":[],"memory_ids_rejected":[]}。只有 Memory Card 实际影响代码、接口或分页实现时才能列入 memory_ids_used；被当前证据否定时列入 memory_ids_rejected；仅仅看到卡片不得申报使用。
...
"""
    ...
    memory_section = (
        ...
        "Memory 使用回执：AI_REVIEW_JSON 必须包含 memory_ids_used 和 memory_ids_rejected；"
        "只有真实采用或证伪的 memory_id 才能填写。同站点/同路由的高分 endpoint 可优先做一次有界验证；"
        "跨站策略只能复用通用分页思路，禁止复制其 URL、selector 或认证事实。\n"
    )
    completion = """
...
def _inspect_native_generated_code(
    ...
    from crawler_agent.tools.code_tools import inspect_python_file
    ...
    # The legacy AST keyword checker (including browser_owned_requests) was the
...
def _validate_native_code_result(
    ...
    from crawler_agent.tools.code_tools import check_data_file, preview_data_file
    ...
    emit_memory_feedback(
        logger,
        code_memory_receipts(ai_review),
        stage="ai_review",
    )
...
def _run_pi_coding_agent(initial_state: CodeAgentState) -> CodeAgentState:
    ...
    from crawler_agent.runtimes.pi_code_runtime import run_pi_coding_agent
    from crawler_agent.tools.code_tools import get_workspace
...
def _run_code_pipeline_impl(
    ...
    from crawler_agent.tools.code_tools import read_text_file, set_workspace
