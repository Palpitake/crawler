...
def normalize_auth_facts(
    ...
    """Compatibility facade for the deterministic authentication subsystem."""
    from crawler_agent.auth.service import evaluate_auth_facts

    return evaluate_auth_facts(parser, pipeline_info, previous)
...
def code_entry_mode(parser: Mapping[str, Any], auth_facts: Mapping[str, Any]) -> str:
    ...
    from crawler_agent.auth.models import AUTH_BLOCKING_STATES

    unresolved_auth = str(auth_facts.get("state") or "unknown") in AUTH_BLOCKING_STATES
    if unresolved_auth:
        return "probe"
