...
class RagService:
    ...
    def search_state(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for rank, card in enumerate(cards["supervisor"], start=1):
            log_event(logger, "rag.select", status="selected", memory_id=card.get("memory_id"), rank=rank, score=card.get("match_score"), memory_type=card.get("memory_type"), match_reasons=card.get("match_reason"), requires_validation=card.get("requires_validation"))
        return result
    ...
    def commit_state(self, state: Dict[str, Any], builds: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            ...
            log_event(logger, "rag.upsert", status="success", backend=result.get("backend"), quality=quality, memories=len(payload.get("memories") or []), endpoints=len(payload.get("endpoints") or []), failure=bool(payload.get("failure")), latency_ms=int((time.monotonic()-started)*1000))
            return result
        except Exception as exc:
            if not self.config.fail_open:
                raise
            fallback_result = self._fallback.commit_state(payload)
            log_event(logger, "rag.upsert", level="WARNING", status="degraded", backend="jsonl_fallback", quality=quality, error_type="rag_backend_unavailable", reason=str(exc), latency_ms=int((time.monotonic()-started)*1000))
            return {**fallback_result, "warning": str(exc)}
