...
class MySQLRagRepository:
    ...
    def commit_state(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"ok": True, "backend": self.backend_name, "memory_ids": memory_ids}
    ...
    def _upsert_memory(self, cursor: Any, value: Dict[str, Any]) -> int:
        sql = """
            ...
            ON DUPLICATE KEY UPDATE
                status = VALUES(status), source_kind = VALUES(source_kind),
                summary = VALUES(summary), searchable_text = VALUES(searchable_text),
                facts = VALUES(facts), metrics = VALUES(metrics),
                ...
                fresh_until = VALUES(fresh_until), schema_version = VALUES(schema_version),
